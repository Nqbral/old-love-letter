...
export default function PlayModalBaron({
  handleClose,
  gameState,
  playersParsed,
  myPlayer,
  ...
  return (
    <ModalTemplate>
      <div className="flex w-2xl flex-col items-center gap-6 text-center">
        <h2 className="text-secondary-hover pb-2 text-2xl">Jouer le Baron</h2>
        {noPlayerAvailable ? (
          <>
            <p className="text-primary-hover">
              Tous les joueurs encore en lice sont protégés par une{' '}
              <span className="font-bold">Servante</span>. Jouer le baron n'aura
              aucun effet.
            </p>
            <div>
            ...
            <ul className="flex w-64 flex-col rounded-lg">
              {listPlayers.map((player, index) => {
                if (player.alive && player.id != myPlayer.id) {
                  ...
                  return (
                    <li
                      key={'select-player-baron-' + player.id}
                ...
                return <div key={'select-player-baron-' + player.id} />;
              })}
            </ul>
            {errorMessage != '' && (
